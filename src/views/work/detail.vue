...
import { definitionStart, getFlowViewer, getProcessVariables, readXml, readImage } from '@/api/workflow/definition'
import {  processDetail } from '@/api/workflow/bpmProcDef'
import { getDetailInstance, complete, transfer, rejectTask } from '@/api/workflow/task'
...
export default {
  ...
  created() {
    ...
    this.taskForm.innerProcDefId = this.$route.query && this.$route.query.innerProcDefId;
  ...
  methods: {
    getImage(){
      console.log("获取流程图片param", this.taskForm.innerProcDefId)
      readImage(this.taskForm.innerProcDefId).then(res => {
        let blob = new Blob([res.data], { type: res.data.type });
        this.imgUrl = URL.createObjectURL(blob);

        // this.imgUrl = res
        console.log("获取流程图片返回", this.imgUrl)
      })
    },
    /** 查询部门下拉树结构 */
    getTreeSelect() {
    ...
    getFlowRecordList(procInsId, deployId) {
      ...
      console.log("查询流程实例详情", params)
      getDetailInstance(Number(procInsId)).then(res => {
        this.flowRecordList = res.data.flowList;
        console.log("流程流转记录this.flowRecordList=====", this.flowRecordList)
    ...
    handleComplete() {
      let param = {
        // copyUserIds: '1538779500142526465', //抄送人ID   此处使用的是 测试二号
        copyUserIds: '1538774400275763202', //抄送人ID   此处使用的是 测试
        handlerComment: this.taskForm.comment ,
        taskRecordId: this.taskForm.taskId 
      }
      console.log("审批通过param", param)
      this.$refs['taskForm'].validate(valid => {
        if (valid) {
          complete(param).then(response => {
    ...
    handleTransfer(){
      let param = {
        ...
        handlerComment: this.taskForm.comment,
        taskRecordId: this.taskForm.taskId,
        // userId: '1538779500142526465', //转办人ID   此处使用的是 测试二号
        userId: '1551861898397159425', //转办人ID   此处使用的是 fangyongzhi

      }
      console.log("转办param", param)
      this.$refs["taskForm"].validate(valid => {
        if (valid) {
          transfer(param).then(res => {
